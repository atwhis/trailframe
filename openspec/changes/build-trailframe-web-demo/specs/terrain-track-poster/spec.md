...
系统 SHALL 生成约 1600×2400 的竖版海报，包含地形底图、轨迹、海拔曲线、总距离、累计爬升、累计下降和最高海拔，并在页面中预览最终生成的图片。
...
系统 SHALL 默认检索轨迹几何周边 5 公里范围内具有名称和海拔的 OpenStreetMap `natural=peak`，按高度并兼顾距离选取至多 5 座标注在海报地图区域。

#### Scenario: Nearby peaks are available
- **WHEN** 高峰服务返回轨迹周边多个有效山峰
- **THEN** 海报标注至多 5 座代表性高峰的名称与海拔
